...
export const employeeByIdPUT = async (full_name, identification_number, phone, charge_id, company_id, user_id, id) => {
    if(!full_name || !identification_number || !phone || !charge_id || !company_id || !user_id) {
        throw new Error('Los campos son obligatorios')
    }

    if(isNaN(user_id) || isNaN(charge_id) || isNaN(company_id)) {
        throw new Error('el dato a buscar tiene que ser un numero valido')
    };

    const data = await putEmployeesById(full_name, identification_number, phone, charge_id, company_id, user_id, id)
    if(!data) {
